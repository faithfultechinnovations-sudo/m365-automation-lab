...
Operator provides required identity inputs (UPN, display name, etc.)

Script validates Graph connection and tenant

Script checks if user already exists (idempotency guard)

If user exists → exits safely
...
Licensing flows automatically via group membership

Transcript log is written
